...
// requirements stage's own state+history+explain surface, read from the same
// `crate::artifacts` store `/artifacts` browses (the `requirements`/
// `acceptance_criteria` kinds, most-recently-recorded row per issue -- there is no
// separate requirements table, per AIR-3's "one artifact store" constraint) and
// `eventlog::recent_events` (`clarification_raised`, already recorded whenever
...
/// Latest recorded artifact of `kind` for `issue_id` (an issue id doubles as its own
/// cycle id -- see `crate::artifacts`'s doc comment), or `None` if the stage hasn't
/// recorded one yet. `list_for_cycle` returns oldest-first, so the last matching row
...
/// (`{"schema_version": 1, "<field>": [...]}`, see `mcp::execute_pipeline_tool`) back
/// into its item array, ignoring the wrapper.
fn artifact_items(
    workflow_dir: &std::path::Path,
    row: &artifacts::ArtifactRow,
    field: &str,
) -> Vec<serde_json::Value> {
    artifacts::read_content(workflow_dir, row)
        .ok()
...
async fn requirements_page(
    ...
    let requirements_artifact = latest_artifact(&db, &issue_id, "requirements");
    let requirements: Vec<serde_json::Value> = requirements_artifact
        .as_ref()
        .map(|row| artifact_items(&state.workflow_dir, row, "requirements"))
        .unwrap_or_default();
    let acceptance_criteria_artifact = latest_artifact(&db, &issue_id, "acceptance_criteria");
    let acceptance_criteria: Vec<serde_json::Value> = acceptance_criteria_artifact
        .as_ref()
        .map(|row| artifact_items(&state.workflow_dir, row, "acceptance_criteria"))
        .unwrap_or_default();
