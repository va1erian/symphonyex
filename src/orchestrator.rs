...
async fn run_inner(
    ...
    let db_path = shared.config.workflow_dir.join(crate::eventlog::DB_FILENAME);
...
fn write_report(report_path: &Path, workflow_path: &Path, db_path: &Path, state: &OrchestratorState) {
    if let Err(e) =
