...
/// The delivery pipeline's own tool source (AIR-4), independent of tracker kind or
/// repo host -- `record_requirements`, `record_acceptance_criteria` and
/// `raise_clarification` are properties of the pipeline stage running, not of any
/// particular tracker/host integration, so (like the "PR-based branch workflow" note
/// on `route_call` explains for `open_pull_request`) they get their own tool source
/// rather than being bolted onto `TrackerAdapter` or `RepoHost`. Only constructed when
/// `pipeline.enabled` (main.rs), so a project not using the pipeline never sees these
/// tools at all.
///
/// `record_requirements`/`record_acceptance_criteria` store through `crate::artifacts`
/// (AIR-3's canonical artifact store, see that module's doc comment) rather than
/// keeping a second, competing artifacts table -- same shape as `ArtifactsWiring`, so
/// both fields travel together the same way.
...
/// or the code host -- checked first since it never overlaps with the other two),
/// `open_pull_request`/`attach_evidence` from `repo_host` when `repo.pull_request` is
/// enabled, or (AIR-4) the delivery pipeline's own `record_requirements`/
/// `record_acceptance_criteria`/`raise_clarification` when `pipeline.enabled`. Each is
/// a property of a different thing (the tracker, the code host, the pipeline stage
...
/// `record_requirements`/`record_acceptance_criteria` each do a light shape check
/// (every item has its required string/array fields) -- schema conformance for the
/// artifact itself is enforced here so a malformed entry is rejected before it's ever
/// persisted, rather than trusting the agent's JSON blindly -- then store the
/// validated set through `crate::artifacts` (AIR-3's canonical artifact store, see its
/// doc comment: "before adding a concept, try to express with an existing one" --
/// there is exactly one artifacts table, not a second one just for these two kinds).
/// The validated array is wrapped as `{"schema_version": 1, "requirements": [...]}` /
/// `{"schema_version": 1, "acceptance_criteria": [...]}` since `crate::artifacts`'s
/// `validate_structured` requires a JSON object carrying an integer `schema_version`
/// field, not a bare array.
async fn execute_pipeline_tool(
    ctx: &PipelineToolCtx,
    name: &str,
    arguments: Value,
    issue_id: &str,
    ...
    match name {
        "record_requirements" => {
            ...
            let count = items.len();
            let content = json!({"schema_version": 1, "requirements": items}).to_string();
            let result = artifacts::execute_tool(
                ...
                json!({
                    ...
                    "content": content,
                    "summary": format!("{count} requirement(s)")
                }),
            )
            .await;
            if result.success {
                ToolResult::ok(format!("Recorded {count} requirement(s)."))
            } else {
                ToolResult::error(format!("failed to record requirements: {}", result.content))
            }
        }
        "record_acceptance_criteria" => {
            ...
            let content = json!({"schema_version": 1, "acceptance_criteria": items}).to_string();
            let result = artifacts::execute_tool(
                &ctx.db_path,
                &ctx.workflow_dir,
                workspace_dir,
            ...
            .await;
            if result.success {
                ToolResult::ok("Recorded acceptance criteria.".to_string())
            } else {
                ToolResult::error(format!(
                    "failed to record acceptance criteria: {}",
                    result.content
                ))
            }
        }
...
async fn route_call(
    ...
    if tracker_names.contains(name) {
        ...
        match pipeline {
            Some(ctx) => execute_pipeline_tool(ctx, name, arguments, issue_id, workspace_dir).await,
...
mod tests {
    ...
    fn pipeline_ctx(dir: &std::path::Path) -> PipelineToolCtx {
        PipelineToolCtx {
            db_path: dir.join("symphony.db"),
    ...
    async fn record_requirements_then_record_acceptance_criteria_round_trip_via_artifacts() {
        ...
        // Stored through `crate::artifacts` (AIR-3's canonical store), not a second
        // artifacts table -- fetch the recorded rows and parse their `content` blob.
        let rows = artifacts::list_for_cycle(&ctx.db_path, "42");
        let reqs_row = rows.iter().find(|r| r.kind == "requirements").unwrap();
        let reqs_bytes = artifacts::read_content(&ctx.workflow_dir, reqs_row).unwrap();
        let reqs: Value = serde_json::from_slice(&reqs_bytes).unwrap();
        assert_eq!(reqs["schema_version"], 1);
        let parsed = reqs["requirements"].as_array().unwrap();
        assert_eq!(parsed.len(), 3);
        assert_eq!(parsed[0]["id"], "R1");
        assert_eq!(parsed[1]["constraint"], "performance");
        assert_eq!(parsed[2]["dependency"], "DEMO-2");

        let acs_row = rows.iter().find(|r| r.kind == "acceptance_criteria").unwrap();
        let acs_bytes = artifacts::read_content(&ctx.workflow_dir, acs_row).unwrap();
        let acs: Value = serde_json::from_slice(&acs_bytes).unwrap();
        assert_eq!(acs["schema_version"], 1);
        let parsed_ac = acs["acceptance_criteria"].as_array().unwrap();
    ...
    async fn raise_clarification_requires_question_and_blocking() {
        ...
        std::fs::create_dir_all(&workspace).unwrap();
        let ctx = pipeline_ctx(dir.path());
        let missing_blocking = execute_pipeline_tool(
            &ctx,
            "raise_clarification",
            json!({"question": "q?"}),
            "1",
