...
pub fn check_after_turn(
    ...
    if let Some(limits) = stage_limits
        && let Some(reason) = exceeded_reason(stage_usage, limits, ctx.model_key, ctx.pricing, currency)
    {
        return Some(Verdict {
            scope: Scope::Stage,
    ...
    if let Some(limits) = &ctx.budgets.cycle
        && let Some(reason) = exceeded_reason(cycle_usage, limits, ctx.model_key, ctx.pricing, currency)
    {
        return Some(Verdict {
            scope: Scope::Cycle,
            action: ctx.budgets.on_exceeded,
...
mod tests {
    ...
    fn compute_cost_is_unknown_for_an_unpriced_model_never_zero() {
        ...
        let cost = compute_cost(&usage(1_000_000, 1_000_000), "claude/some-future-model", &pricing);
    ...
    fn check_after_turn_flags_cost_exceeded_only_when_model_is_priced() {
        ...
        let verdict =
            check_after_turn(&ctx_priced, &usage(0, 0), &usage(200_000, 0)).unwrap();
        assert_eq!(verdict.scope, Scope::Cycle);
    }
